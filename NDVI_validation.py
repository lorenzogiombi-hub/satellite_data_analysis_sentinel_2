...
# Create a dictionary between months <--> ndvi_clim_mean
MODIS_REFERENCE = dict(zip(
    _clim["month"].astype(int),
    _clim["ndvi_clim_mean"]
))                                     # MODIS_REFERENCE --> {1: -0.0123312530059823, 2: 0.0142558373260579, 3: 0.050702084184513, 4: 0.1291575927462574, 5: 0.2973686354416904, 6: 0.3499374589566191, 7: 0.3504412554593304, 8: 0.334844568885483, 9: 0.3007607638933458, 10: 0.2309925399591588, 11: 0.0797843832623584, 12: 0.0373442112613243}

# Create a dictionary between months <--> ndvi_clim_std
MODIS_REFERENCE_STD = dict(zip(
    _clim["month"].astype(int),
    _clim["ndvi_clim_std"]
))
...
def compute_phenology(dates_dt, mean_ndvi_series, threshold=0.4):
    ...
    # Determine the green_up as the first date when mean_ndvi overcomes the threshold (before peak of ndvi)
    green_up = None
    for i in range(1, peak_idx + 1):
        if mean_ndvi_series[i - 1] < threshold <= mean_ndvi_series[i]:
            green_up = dates_dt[i]
            break
    # Determine the senescence as the first date when mean_ndvi drops below the threshold (after peak of ndvi)
    senescence = None
...
# print(dates_str) --> ['Helsinki_2025_01_18', 'Helsinki_2025_02_25', 'Helsinki_2025_03_14', 'Helsinki_2025_04_15', 'Helsinki_2025_05_16', 'Helsinki_2025_06_04', 'Helsinki_2025_07_17', 'Helsinki_2025_08_04', 'Helsinki_2025_09_28', 'Helsinki_2025_10_18', 'Helsinki_2025_11_14', 'Helsinki_2025_12_27']
...
pixel_area_km2 = (pixel_sizes[0] ** 2) / 1e6   # Area in one pixel. Assumes consistent resolution
...
for ndvi in ndvi_frames:     # Remember each frame corresponds to a month
...
for dt, mn in zip(dates_dt, mean_ndvi):
    ref      = MODIS_REFERENCE.get(dt.month)
    ref_std  = MODIS_REFERENCE_STD.get(dt.month)
    if ref is not None:
        diff = mn - ref
...
ref_months = sorted(MODIS_REFERENCE.keys())
ref_vals   = [MODIS_REFERENCE[m] for m in ref_months]
